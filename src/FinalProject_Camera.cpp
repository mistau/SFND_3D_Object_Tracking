...
#ifdef ITERATE
    ofstream table;
    table.open("../report/table.md");
    table << "| Detector / Descriptor |  01  |  02  |   03 |  04  |  05  |  06  |  07  |  08  |  09  |  10  |  11  |  12  |  13  |  14  |  15  |  16  | 17  | 18  |" << endl;
    table << "| --------------------- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | ---- | --- | ---- |" << endl;
    
    ofstream data;
    data.open("../data/results.txt");
    
    const vector<string> detectors = {"SHITOMASI", "HARRIS", "FAST", "BRISK", "ORB", "AKAZE", "SIFT"};
#else
    const vector<string> detectors = {"BRISK"};
#endif
       ...
       for(auto descriptorType: descriptors)
        {
            ...
            // misc
            double ttc_values[imgEndIndex+1];   // array to keep our results
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
            {
                ...
                // remove keypoints which are not on vehicle!
                    ...
                    for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
                    {
                        ...
                        if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                        {
                            ...
                            if (bVis)
                            {
                                ...
                                char str[200];
                                sprintf(str, "TTC Lidar : %f s, TTC Camera : %f s (%ld)", ttcLidar, ttcCamera, imgIndex);
            ...
            table << "| " << detectorType << " / " << descriptorType << " "; 
            for(int idx = 1; idx<=imgEndIndex; ++idx)
                table << "| " << ttc_values[idx] << " ";
            table << endl;

            // generate date file for python
            data << detectorType << "_" << descriptorType << " ";
            for(int idx = 1; idx<=imgEndIndex; ++idx)
                data << " " << ttc_values[idx];
            data << endl;
#endif
